...
def parse_arguments():
    ...
    parser.add_argument('--template-variables-path',
                        help='path or URL to read jinja2 template variables from')
    ...
    args_dict['requests_cache_path'] = str(output_dir.joinpath('requests-cache'))
...
def get_metadata_and_variables(args):
    ...
    user_variables = dict()
    path = args.template_variables_path
    if path:
        try:
            user_variables = read_json(path)
            assert isinstance(user_variables, dict)
        except Exception:
            logging.exception(f'Error reading template variables from {path}')
    logging.info(f'Read the following user-provided templating variables from {path}\n'
